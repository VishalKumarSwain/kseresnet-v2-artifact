"""
V5: same as V4's offline pairwise ranking training, but fail/pass pairs are
sampled ONLY within the same road generator (never cross-generator). This
targets the diagnosed V4 issue: pairs sampled from the mixed pool let the
model partly exploit cross-generator base fail-rate differences (Ambiegen
45% vs Frenetic 37% vs FreneticV 33%) rather than learning genuine
within-generator discrimination, which showed up as V4 being significantly
*worse* than V2 on Ambiegen and FreneticV when evaluated generator-by-generator.
"""
import argparse
import time
import numpy as np
...
def main(args):
    ...
    X, y, gen = data["X"], data["y"], data["gen"]
    ...
    generators = sorted(set(gen.tolist()) - {"unknown"})
    gen_fail_idx = {g: np.where((y == 1.0) & (gen == g))[0] for g in generators}
    gen_pass_idx = {g: np.where((y == 0.0) & (gen == g))[0] for g in generators}
    for g in generators:
        print(f"  {g}: fail={len(gen_fail_idx[g])} pass={len(gen_pass_idx[g])}")
    ...
    pairs_per_gen = args.pairs_per_epoch // len(generators)
    ...
    for epoch in range(args.epochs):
        ...
        fi_all, pj_all = [], []
        for g in generators:
            fi_all.append(rng.choice(gen_fail_idx[g], size=pairs_per_gen, replace=True))
            pj_all.append(rng.choice(gen_pass_idx[g], size=pairs_per_gen, replace=True))
        fi = np.concatenate(fi_all)
        pj = np.concatenate(pj_all)
        perm = rng.permutation(len(fi))
        fi, pj = fi[perm], pj[perm]

        epoch_loss = 0.0
        n_batches = 0
        for s in range(0, len(fi), args.batch_size):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--features", default="train_features_gen.npz")
    parser.add_argument("--base-checkpoint", default="crash_model_v2.pth")
    parser.add_argument("--out", default="crash_model_v5.pth")
