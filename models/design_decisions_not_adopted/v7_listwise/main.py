"""
V7: same features/architecture as V4, but trained offline with a listwise
(ListNet-style) ranking loss instead of the pairwise RankNet loss (see
train_listwise_v7.py), warm-started from V2's checkpoint. Robustness check
requested in review: does a listwise objective change the RQ3/RQ4 findings
relative to the pairwise loss used for the proposed model?
"""
import grpc
...
class KSERESNETv7(pb2_grpc.CompetitionToolServicer):
    def __init__(self):
        print("Loading KSERESNET-V7 (offline listwise-ranking-trained) ...")
        self.model = ResNetPredictorV2(in_channels=IN_CH)
        checkpoint = torch.load("crash_model_v7.pth", map_location="cpu")
    ...
    def Name(self, request, context):
        return pb2.NameReply(name="KSERESNET-V7")
...
def serve():
    ...
    pb2_grpc.add_CompetitionToolServicer_to_server(KSERESNETv7(), server)
    server.add_insecure_port("[::]:50051")
    server.start()
    print("KSERESNET-V7 listening on 50051 ...")
