...
the ICST 2026 SDC Testing Tool Competition and \emph{the authors' own
prior work}: this paper is a deliberately self-critical replication and
extension of our own submission, not a promotional one, and we evaluate
it exclusively under the organizers' independent protocol rather than our
own self-reported results throughout. Our contribution is the
diagnosis and its empirical scale, not a new technique: we use a standard,
off-the-shelf pairwise ranking loss as a diagnostic instrument, testing
whether closing the training/evaluation objective mismatch -- with no
novel method and no architecture change -- is sufficient to close most of
the tool's performance gap.
...
tools, KSERESNET \citep{swain2026kseresnet}, is the authors' own
competition submission; we state this plainly here, at the outset, rather
than later, because it is the reason this paper exists in its present
form. This paper is a self-critical replication and extension of that
submission, not a promotional one: we hold KSERESNET to the same
independent-evaluation standard as every competing tool throughout
(Section~\ref{sec:methodology}), and its self-reported figures below are
reported only to be immediately superseded by the organizers' independent
measurement. KSERESNET reported 95.27\% classification
...
0.625). As the original submission's authors, we checked our own
development artifacts for the cause rather than leaving it unexplained
(Section~\ref{sec:rq1} gives the details); the explanation we found
...
and does closing it require a new technique, or does a correctly targeted
standard one suffice? We deliberately choose the latter as a diagnostic
strategy: if a well-established, off-the-shelf pairwise ranking loss
\citep{burges2005learning} closes most of the gap without any change to
the model's architecture or features, that is evidence for the diagnosis
itself, not a contribution to learning-to-rank methodology.

This framing raises a question we address directly rather than leave
implicit: is a diagnosis, plus the application of a two-decade-old
ranking-loss technique, sufficient contribution for a journal-length
empirical study? We argue yes, on three grounds this paper substantiates
rather than merely asserts. First, empirical standards for the field
explicitly recognize diagnostic and replication studies as a distinct,
legitimate contribution type with their own evaluation criteria -- rigor
and evidentiary strength, not technical novelty -- rather than a lesser
substitute for a new-method paper \citep{ralph2020empirical}; the
standard we hold ourselves to throughout (Section~\ref{sec:methodology})
is the former. Second, precedent in adjacent ML-systems research argues
that measurement and diagnostic work on widely deployed but
under-scrutinized techniques is frequently more consequential than
incremental methodological novelty, precisely because such techniques are
already in production and their failure modes are not
\citep{sculley2015hidden}; a pointwise/ranking objective mismatch that
recurs across every submitted competition tool (Table~\ref{tab:ablation})
is exactly this kind of already-deployed, previously unscrutinized
failure mode. Third, and most concretely, this paper's contribution is
not exhausted by ``diagnosis plus standard fix'': RQ4 reports a masking
effect -- aggregate improvement obscuring generator-level regressions --
that is, to our knowledge, new to the SDC test-prioritization literature
and is not implied by either the diagnosis or the fix in isolation; the
four robustness checks (Section~\ref{sec:rq3-robustness}) are themselves
negative results that rule out simpler explanations, independent of
whether the fix itself is novel; and the cross-edition comparison (RQ5)
and fully reproducible harness are contributions a purely
technique-focused paper would have no reason to make. We treat the
diagnosis, its empirical scale, and these follow-on findings collectively
as the contribution, with the pairwise loss serving throughout as the
diagnostic instrument rather than as the result being claimed.

Using the
organizers' own evaluation protocol -- reconstructed
...
\item An empirical demonstration, using a standard pairwise ranking loss
  \citep{burges2005learning} rather than a new method, that closing the
  training/evaluation objective mismatch raises mean APFD from 0.625
  to 0.794 on a leakage-free held-out split (RQ3), evaluated with
  non-parametric significance tests and effect sizes, together with four
  robustness checks (including a listwise alternative) that rule out
  simpler explanations for the remaining gap to the state of the art.
...
We do not propose a new ranking-loss formulation or architecture: the
pairwise loss we use in RQ3 (Section~\ref{sec:proposed}) is the standard
formulation from \citet{burges2005learning}, applied unmodified. The
contribution is diagnostic -- showing that this well-known
technique-diagnosis mismatch is present, large, and measurable in a
concrete class of deployed SDC tools, not previously documented for this
domain -- rather than methodological.
...
\citep{holm1979simple} within that family. $N$ in each protocol
(75 or 160 trials, below) was fixed by the resampling design -- the
subject sizes and repeats used to build the trial grid -- rather than by
a power analysis targeting a specific effect size in advance; post hoc,
the range of $\hat{A}_{12}$ values this design actually detects across
our comparisons (0.050--0.997, including both the large effects we call
real and the near-null 0.431 we call not significant) indicates it was
adequately powered for the effects observed, including distinguishing a
genuine null from a real one.
...
different ways. Because we are the original submission's authors, we
checked our own development artifacts for the cause rather than treating
it as unexplained. We found three concrete, non-speculative differences
between the original self-evaluation and the organizers' protocol, any one
of which would inflate a self-reported figure: (i) the self-evaluation
script we used scored the model on the exact same 956-test file the model
was trained on, with no held-out split at all -- direct train/evaluation
leakage; (ii) it computed a single APFD over that entire fixed set in one
pass, not the organizers' repeated-trial, random-subject-sampling protocol
(Section~\ref{sec:harness}); and (iii) that 956-test file is a small local
development sample bundled with the competition repository, not the
32{,}580-test corpus the organizers use for official scoring. We do not
disentangle the individual contribution of each factor -- doing so would
require retraining under each condition in isolation, which is outside
this paper's scope -- but we no longer treat the gap as unexplained, and
it is why every other number in this paper is drawn exclusively from
independently reproduced evaluations (Section~\ref{sec:harness}). The
second, and the one this paper diagnoses
...
\textbf{Is the remaining gap to RoadFury practically meaningful?} We
state this plainly rather than leaving it implicit. The gap is real, not
deployment noise: $\hat{A}_{12}=0.750$ means RoadFury outperforms the
proposed model in a clear majority of trials, not merely on a fragile
average, and this holds across the full 10--200 size range we test (mean
per-size APFD gap ranges from 0.005 at size 30 to 0.017 at size 50, with
no size at which the proposed model wins outright). But its
\emph{practical} magnitude is small: translating the aggregate 0.009 APFD
gap into fault-detection position using $\mathrm{APFD}=1-\frac{\sum
TF_i}{nm}+\frac{1}{2n}$ gives an average per-fault detection delay of
0.009$\times n$ positions at subject size $n$ -- about 0.1 positions at
$n=10$, rising to about 1.2 positions at the largest size we test
($n=200$). That is, RoadFury detects the average fault roughly one test
position earlier in a 200-test suite, and less at smaller sizes. We do
not think this magnitude, on its own, should be the deciding factor in a
practitioner's tool choice: the proposed model's more than $2\times$
faster prioritization time (0.370s vs.\ 0.804s, Table~\ref{tab:main}) is
arguably just as relevant to that decision. What we do claim is narrower
and better supported: the gap is statistically real, small in absolute
terms, and one of several factors -- not the only one -- a practitioner
would weigh.
...
tests below use $N=160$ trials, subject sizes 10--200). $^\dagger$APFD for
these two tools is averaged over only the 31\% of trials they completed,
concentrated at easier, smaller subject sizes (Section~\ref{sec:rq3}); it
is \emph{not} comparable to the other rows, which are averaged over all
160 trials including the hardest, and is reported for context only, not
as a validated head-to-head figure.}
\label{tab:main}
\begin{tabular}{lrrrr}
\toprule
...
ITEP4SDC$^\dagger$ \citep{gullu2026itep4sdc} & 0.788 & 0.857 & 0.184 & \textbf{31\%} \\
RTE4SDC \citep{maugeri2026rte4sdc} & 0.779 & 0.836 & 0.244 & 100\% \\
EagleSemble$^\dagger$ \citep{aquilinoeaglesemble} & 0.777 & 0.851 & 0.477 & \textbf{31\%} \\
...
\citep{vargha2000critique}.

\textbf{Does this have practical consequence for a practitioner's tool
choice?} We answer this directly rather than leaving it as a caveat. The
effect is statistically real -- $\hat{A}_{12}=0.253$ and $0.353$ indicate
a consistent shift, not noise in a handful of trials -- but its practical
magnitude, using the same fault-detection-position translation as
Section~\ref{sec:rq3}, is small and size-dependent: at subject size 80
(the largest size in this protocol), the average fault is detected about
1.1 positions later on Ambiegen and 0.9 positions later on FreneticV; at
size 10 the effect is close to zero on both (Table~\ref{tab:gen} reflects
sizes up to 80 only). We do not think a delay of roughly one test
position in an 80-test suite is, on its own, grounds for a practitioner
to prefer the intermediate model over the proposed one -- the aggregate
RQ3 improvement (0.625 to 0.794 APFD) is far larger and holds across the
full mixed-generator pool. What this finding does mean is narrower: the
aggregate RQ3 improvement should not be read as a uniform
within-generator improvement, and any deployment that runs exclusively on
a single generator's tests should be aware the proposed model is not
strictly dominant there.
...
\textbf{Example-level error analysis.} To move beyond the aggregate
statistic, we scored every held-out test in each generator with both
models directly and examined individual rank changes for FAIL tests.
First, we looked for a geometric signature that distinguishes demoted
FAIL tests from the rest: mean and maximum $|\text{curvature}|$ and mean
$|\text{angle\_change}|$ for the most-demoted decile of FAIL tests are
statistically indistinguishable from the least-demoted decile in every
generator (e.g., Ambiegen: mean $|\text{curvature}|=0.0305$ vs.\ $0.0282$;
FreneticV: $0.0248$ vs.\ $0.0231$) -- demotion is not concentrated in an
identifiable road-shape subtype we can characterize from these features.
Second, we categorized \emph{why} each demoted FAIL test's rank got worse:
for every FAIL test the intermediate model ranked ahead of the proposed
model, we identified which specific tests newly overtake it and whether
they are themselves FAIL (a reordering \emph{within} the correctly-flagged
tests, largely harmless to APFD) or PASS (a genuine rank inversion that
directly costs APFD). Both occur, and their mix varies by generator: 74\%
of Ambiegen's demoted FAIL tests, 58\% of FreneticV's, and 51\% of
Frenetic's are overtaken by at least one PASS test. Two concrete Ambiegen
examples illustrate the range: test \texttt{mt/set\_8/ambiegen/odr/test-1}
drops from rank 392 to 661 (of 2{,}523), but 264 of its 269 overtakers are
themselves FAIL tests -- the ranking-trained model has learned to
prioritize other failures even more aggressively, which barely affects
APFD. Test \texttt{mt/set\_12/ambiegen/test-446} drops from rank 930 to
1190, but 100 of its 260 overtakers are PASS tests that the intermediate
model had correctly ranked behind it -- a genuine inversion. We take this
as evidence that the RQ4 regression is a mix of a mostly benign
redistribution of confidence among true failures and a smaller but real
rate of fail/pass rank inversions, rather than a single mechanistic
failure mode; we did not identify a feature-level property that predicts
which case a given test falls into, and leave a more targeted diagnosis
(e.g., attention or saliency analysis of the network itself) to future
work.
...
head-to-head claims. Relatedly, we address the practical (as opposed to
statistical) significance of the RQ4 per-generator regressions directly
in Section~\ref{sec:rq4}, rather than treating it only as a threat: the
effect sizes are real, but the translated practical magnitude (roughly
one test position at the largest subject size tested) is small.
\textbf{External.} All experiments use a single
...
is, on its own, a theoretical argument for external validity, not an
empirical one. We supplement it with a lightweight empirical check rather
than leaving it purely theoretical: using the identical two-phase recipe
(pointwise pretrain, then pairwise ranking-loss fine-tune,
Section~\ref{sec:proposed}) on an unrelated, non-SDC, synthetic binary
classification dataset (4{,}000 samples, 20 features, no relation to
roads, simulators, or SensoDat), with a small MLP in place of the
SE-ResNet (architecture is incidental to the mechanism under test), the
same qualitative pattern recurs: held-out classification accuracy is
essentially unchanged by the switch (0.806 vs.\ 0.807), while a
trial-sampled, APFD-computed ranking metric improves consistently across
every one of 6 random seeds we tried ($\hat{A}_{12}$ ranging 0.534--0.691,
$N=160$ trials per seed, $p<0.05$ in 5 of 6 seeds). The magnitude is
small -- consistent with this paper's own isolated, same-features
training-objective effect (RQ2: 0.759 to 0.794, Table~\ref{tab:ablation})
rather than the much larger full-pipeline gain, most of which RQ2
attributes to features, not the training objective -- and this toy check
is not a substitute for a second SDC corpus: it has no road geometry, no
simulator, and cannot speak to RQ4's generator-specific finding at all.
It is, however, real, non-cherry-picked evidence (we report all 6 seeds
we ran, not a selected subset) that the accuracy/ranking decoupling this
paper diagnoses is not an artifact specific to SensoDat or to SDC
trajectory features. We single out RQ4 (the per-generator regression) as the finding least likely
...
training objective) and, using nothing more than a standard, off-the-shelf
pairwise ranking loss \citep{burges2005learning} rather than a new
technique, substantially repairable (retraining with it closes the gap to
the state of the art from 0.18 to 0.009 APFD) without discarding the
underlying architecture -- evidence for the diagnosis, since a
well-targeted standard remedy would not be expected to work this well if
the underlying problem were something else. This repair is not,
...
SDC-specific finding in this paper is established on SensoDat alone; a
lightweight, non-SDC toy check (Section~\ref{sec:threats}) supports the
generality of the underlying accuracy/ranking mechanism but is not a
substitute for that replication. A further direction is integrating a
generative adversarial network (GAN) into the pipeline as a test
\emph{generator} targeting the ranking blind spots RQ4 identifies --
producing new road scenarios adversarially selected to be
hard-to-rank for the current model, rather than relying only on the
existing test generators' outputs. This is a generation-side extension,
complementary to the prioritization-side contribution of this paper
rather than a modification of it.
