...
one (Section~\ref{sec:threats}). More directly: Section~\ref{sec:harness}
shows that every one of the four official tools we did not build ourselves
(RoadFury, EagleSemble, RTE4SDC, ITEP4SDC) reproduces its
independently published APFD within 0.03 when run against this corpus,
and the two tools with known reliability defects reproduce the identical
root-cause failure. Since those tools' behavior is determined entirely by
the input data, this match is direct evidence that our reconstructed
corpus is functionally faithful to the one underlying the organizers'
evaluation, independent of the disclosed FreneticV count difference. We
split the full 36{,}006-test corpus
