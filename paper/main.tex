...
\institute{[Affiliations withheld for drafting]}
...
the real, 36{,}006-test SensoDat corpus, validate it against the
...
leakage-free 80/20 split, and three follow-up robustness checks (extended
training, generator-stratified sampling, a Transformer backbone) to test
...
APFD. Two further findings qualify this result: the improvement partly
masks generator-specific regressions invisible in aggregate metrics, and
three follow-up experiments testing plausible alternative explanations
(more training, stratified sampling, a Transformer backbone) each produced
...
objective, but aggregate improvement can obscure subgroup regressions that
prior evaluations of this class of tool do not examine.
...
0.62 \citep{aryan2026icst} -- barely better than a random ordering (0.50)
and far behind the eventual winner (0.80). This paper's motivation is
...
0.62), which we do not attempt to explain (we did not have access to the
...
against three plausible alternative explanations. We also surface an
...
\item A ranking-aware retraining procedure that raises mean APFD from 0.625
  ...
  significance tests and effect sizes (RQ3), together with three
  robustness checks that rule out simpler alternative explanations for the
  remaining gap to the state of the art.
\item A finding that aggregate APFD improvement can mask road-generator-level
...
-- has received comparatively less attention. \citet{birchler2023single}
...
evaluate on the full, real, 36{,}006-test SensoDat corpus
...
the ablation (RQ2), the ranking-loss retraining and its three robustness
...
The baseline model's SE-attention blocks follow the Squeeze-and-Excitation
design of \citet{hu2018squeeze}. The Transformer variant used in one of our
robustness checks (Section~\ref{sec:rq3-robustness}) follows the encoder
architecture of \citet{vaswani2017attention}.
...
\item[RQ3 (Remedy).] Does an offline, ranking-aware training objective
  ...
  a different architecture) account for the same improvement?
...
\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{Framework1.png}
...
underlying the ICST 2026 competition. We reconstructed the full corpus from
raw OpenDRIVE road files and per-test outcome metadata, obtaining
36{,}006 unique test cases (13{,}824 FAIL / 22{,}182 PASS; deduplicated by
each test's embedded identifier) across three test generators -- Ambiegen,
Frenetic, and FreneticV -- exceeding the 32{,}580-test, 36-collection figure
reported in \citet{aryan2026icst} by 3{,}426 tests. We give a full
accounting of this discrepancy rather than treating it as a rounding
matter, since it bears on whether our corpus is a faithful reconstruction
of the organizers' benchmark. Broken down by generator, our local corpus
contains 12{,}496 Ambiegen tests and 12{,}135 Frenetic tests --
\emph{exact} matches to \citet{aryan2026icst}'s Table~II, to the individual
test. The entire 3{,}426-test discrepancy is therefore attributable to
FreneticV alone: 11{,}375 tests locally versus 7{,}949 reported by the
organizers. We cannot identify which specific FreneticV tests the
organizers' narrower sample excludes, since their exact subset was not
published, so we checked instead for evidence of systematic bias: the full
local FreneticV pool has a 33.0\% fail rate (3{,}758/11{,}375), close to
Frenetic's 35.6\% and not dramatically lower or higher in a way that would
suggest the additional tests are disproportionately easy (PASS-heavy) or
hard (FAIL-heavy). This does not rule out subtler distributional
differences we cannot detect without the organizers' exact FreneticV
subset, and we report it as a residual limitation rather than a resolved
one (Section~\ref{sec:threats}). We split the full 36{,}006-test corpus
80/20 (28{,}804 / 7{,}202 tests) with a fixed seed, and used the 20\%
held-out portion \emph{exclusively} for evaluating any model we trained
ourselves, to avoid training/evaluation leakage.

\subsection{Evaluation Harness}
\label{sec:harness}
The organizers' evaluation infrastructure is Docker/PostgreSQL/MongoDB-based
...
retrained offline on the full 28{,}804-test training split with a pairwise
...
convolutions (each followed by batch normalization), a Squeeze-and-Excitation
attention gate \citep{hu2018squeeze} that reweights the 32 channels by a
learned, globally-pooled gate, an additive residual connection, and a final
ReLU. A kernel-2 max-pool halves the sequence length after the first and
second residual blocks only (200$\to$100$\to$50; the third block is followed
directly by flattening). The flattened $32\times50=1{,}600$-dimensional
representation is passed through a fully connected head
(Linear(1600$\to$64)$\to$ReLU$\to$Dropout(0.3)$\to$Linear(64$\to$1)) to
produce a single scalar score, with higher scores indicating higher
...
(20{,}000 pairs/epoch); Section~\ref{sec:rq3-robustness} shows this epoch
...
For every paired comparison we report the mean difference, the Wilcoxon
signed-rank test, and the Vargha--Delaney $\hat{A}_{12}$ effect size. Where
multiple tests support a single research question, we apply a
Holm--Bonferroni correction within that family.
...
initialization). This ablation was run on the full 36{,}006-test corpus
...
fixed improves mean APFD by 0.143 ($\hat{A}_{12}=0.980$,
$p=1.7\times10^{-13}$); adding fine-tuning while holding the feature set
fixed improves it by only 0.028 with weak features and, with the richer
feature set already in place, is not merely non-additive but mildly
\emph{harmful} ($\hat{A}_{12}=0.327$, $p=0.022$). Channel-importance analysis
(zeroing each of the 8 channels and measuring the resulting drop in
fail/pass separation AUC) confirms the mechanism: the two geometry channels
...
APFD from 0.625 to 0.794 ($\hat{A}_{12}=0.997$,
$p=7.6\times10^{-28}$), and $\mathrm{APFD}_C$ from 0.668 to 0.855. The
proposed model statistically exceeds RTE4SDC ($\hat{A}_{12}=0.800$,
$p=5.9\times10^{-14}$) and is within 0.009 APFD of RoadFury, the eventual
competition winner ($\hat{A}_{12}=0.750$, $p=4.2\times10^{-13}$) -- a real
but small remaining gap. All comparisons survive Holm--Bonferroni correction
...
held-out split ($N=7{,}202$ tests, 160 trials, sizes 10--200).}
\label{tab:main}
\begin{tabular}{lrrrr}
\toprule
...
\subsubsection{Robustness checks: three alternative explanations, ruled out}
\label{sec:rq3-robustness}
We tested three plausible alternative paths to closing the remaining gap
...
significant \emph{decrease} (mean APFD $-0.0016$, $p=0.0003$) -- mild
overfitting, not underfitting. (ii) \emph{Generator-stratified pair
sampling} during training, intended to prevent the model from exploiting
cross-generator base failure-rate differences (Ambiegen 45\% vs.\ Frenetic
37\% vs.\ FreneticV 33\%) rather than learning genuine within-generator
discrimination, produced no measurable change ($p=0.11$), which also rules
out that specific mechanism as an explanation for the RQ4 finding below.
(iii) A \emph{Transformer encoder} backbone \citep{vaswani2017attention},
trained under an identical two-phase (pointwise pretrain, then ranking
fine-tune) recipe on the same features, performed substantially
\emph{worse} than the ResNet (mean APFD 0.742 vs.\ 0.794,
$\hat{A}_{12}=0.950$, $p<10^{-25}$) -- architecture family is not, on this
budget, a lever toward closing the gap, even though the competition's
best-performing tool (RoadFury) also uses a Transformer, which we attribute
to differences in feature richness or training scale we did not have access
to reproduce, not to the architecture per se.

\begin{table}[t]
\caption{Alternative design choices tested and not adopted.}
\label{tab:design}
\begin{tabular}{p{3.6cm}p{3.6cm}p{3.6cm}}
\toprule
Design choice & Result vs. proposed model & Decision \\
\midrule
More training epochs (40 vs. 15) & $-0.0016$ APFD, $p=0.0003$ & Not adopted -- mild overfitting \\
Generator-stratified pair sampling & $-0.0004$ APFD, $p=0.11$ (n.s.) & Not adopted -- no effect \\
Transformer backbone (same recipe) & $-0.052$ APFD, $p<10^{-25}$ & Not adopted -- underperforms ResNet \\
\bottomrule
\end{tabular}
\end{table}
...
Table~\ref{tab:ablation}, here resampled per generator under the held-out
protocol), the proposed (ranking-trained) model is significantly
\emph{worse} on Ambiegen
($\hat{A}_{12}=0.353$, $p=0.003$) and FreneticV
($\hat{A}_{12}=0.253$, $p<0.0001$), and not significantly different on
Frenetic ($p=0.28$). In raw APFD these regressions are small (0.004 and
0.008 respectively), and with $N=7{,}202$ tests a p-value this small is
unsurprising for almost any non-zero effect -- so we deliberately report
the effect size, not the p-value, as the basis for calling these
regressions meaningful: $\hat{A}_{12}=0.253$ and $0.353$ both fall at or
beyond the conventional medium-to-large threshold for the Vargha--Delaney
statistic \citep{vargha2000critique}, indicating a consistent, not merely
detectable, shift in the probability that the proposed model ranks a
randomly drawn subject worse than the intermediate model. We report this
as a real but modest finding: it is not, on its own, grounds to prefer the
intermediate model in practice, but it does mean the aggregate RQ3
improvement should not be read as a uniform within-generator improvement.
This is a Simpson's-paradox-like effect: the
...
mechanism is not simply an artifact of pair-sampling imbalance. To our
...
\begin{table}[t]
\caption{Per-generator APFD (mean).}
\label{tab:gen}
\begin{tabular}{lrrr}
\toprule
...
\begin{table}[t]
\caption{Ablation: feature set $\times$ adaptation strategy.}
\label{tab:ablation}
\begin{tabular}{lrr}
\toprule
...
possible. Our reconstructed corpus contains 3{,}426 more FreneticV tests
than the organizers' benchmark (Section~\ref{sec:dataset}); we found no
gross fail-rate anomaly in the additional tests, but cannot rule out subtler
distributional differences without the organizers' exact subset, and this
could in principle shift FreneticV-specific APFD estimates (including the
RQ4 finding) by an amount we cannot bound precisely. \textbf{Internal.} We
...
sample comparable to the fully reliable tools. We mitigate this by
...
their Vargha--Delaney effect sizes ($\hat{A}_{12}=0.253$--$0.353$) meet the
conventional threshold for a medium-to-large effect \citep{vargha2000critique},
not merely because $p<0.05$ under a large sample -- but we do not claim
these regressions are large enough to change a practitioner's tool choice
on their own. \textbf{External.} All experiments use a single dataset
(SensoDat) and a single simulator; generalization to other SDC simulation
platforms is untested. \textbf{Conclusion validity.} We apply
...
however, complete or uniform: it does not survive a per-generator
breakdown for two of three generators, and three plausible follow-up
improvements (more training, stratified sampling, a Transformer backbone)
...
released at [repository URL to be added].
\label{sec:availability}
\textbf{Use of AI tools.} An AI coding assistant (Claude Code) was used to
implement the evaluation harness, training scripts, and statistical
analysis described in this paper, under direct author supervision.
Verification beyond author review consisted of: (i) validating the
evaluation harness itself against the organizers' independently published
results (Section~\ref{sec:harness}) before trusting it for any new
experiment; (ii) computing Wilcoxon signed-rank statistics via SciPy's
standard, independently maintained implementation rather than custom code;
and (iii) manually spot-checking the Vargha--Delaney $\hat{A}_{12}$
effect-size outputs against hand-counted win/loss/tie tallies on a sample
of comparisons. We did not construct a fully independent second
implementation of the statistical pipeline, which we note as a limitation
of our verification process rather than omit.
